...
contract NotesFactory is Ownable {
    using Counters for Counters.Counter;
    
    event NotesContractCreated(address notesContractAddress, address owner);

    struct NotesContractInfo {
    ...
    NotesContractInfo[] public notesContracts;
    mapping(address => address[]) public ownerNotesContracts; // owner => notesContractAddresses
    mapping(address => address) public classToNotesContract; // classAddress => notesContractAddress

    constructor() Ownable(msg.sender) {}

    function createNotesContract(address owner, string memory className, address classAddress) public onlyOwner returns (address) {
        NotesContract newNotesContract = new NotesContract(owner, className);
        address notesContractAddress = address(newNotesContract);
        
        notesContracts.push(NotesContractInfo({
            contractAddress: notesContractAddress,
            owner: owner,
            className: className,
            createdAt: block.timestamp
        }));
        
        ownerNotesContracts[owner].push(notesContractAddress);
        classToNotesContract[classAddress] = notesContractAddress;
        
        emit NotesContractCreated(notesContractAddress, owner);
        
        return notesContractAddress;
    }

    function getNotesContracts() public view returns (NotesContractInfo[] memory) {
        return notesContracts;
    }

    function getOwnerNotesContracts(address owner) public view returns (address[] memory) {
        return ownerNotesContracts[owner];
    }

    function getNotesContractForClass(address classAddress) public view returns (address) {
        return classToNotesContract[classAddress];
    }

    function getNotesContractCount() public view returns (uint256) {
        return notesContracts.length;
    }
} 
